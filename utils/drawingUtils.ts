...
export const magicWandSelect = (
  ...
  const targetColor = pixels[startY][startX];
  const mask = Array(height).fill(false).map(() => Array(width).fill(false));
  const stack: [number, number][] = [[startX, startY]];
  const seen = new Set<string>();
...
export const replaceColor = (
  ...
  return pixels.map(row => row.map(cell => cell === targetColor ? replacementColor : cell));
};

// Helper to initialize empty grid
export const createEmptyGrid = (width: number, height: number): (string | null)[][] => {
...
export const shiftMask = (
  ...
  const newMask = Array(height).fill(false).map(() => Array(width).fill(false));
...
export const rasterizeTransformMask = (
    ...
    const output = baseMask ? baseMask.map(row => [...row]) : Array(gridHeight).fill(false).map(() => Array(gridWidth).fill(false));
