  ...
  return (
    <div className="h-48 bg-gray-900 border-t border-gray-750 flex flex-col">
      ...
      <div className="h-10 border-b border-gray-750 flex items-center px-4 gap-4 bg-gray-850">
        ...
        <button onClick={addFrame} className="text-gray-400 hover:text-white" title="New Frame"><Plus size={16}/></button>
        <button onClick={() => duplicateFrame(currentFrameIndex)} className="text-gray-400 hover:text-white" title="Duplicate Frame"><Copy size={16}/></button>
        <button 
        ...
        {frames.map((frame, index) => (
          <div 
            ...
            className={`
              ...
              ${index === currentFrameIndex ? 'border-indigo-500 shadow-lg shadow-indigo-500/20' : 'border-gray-700 hover:border-gray-500'}
            `}
          >
            <div className="absolute top-1 left-2 text-xs text-gray-500 font-pixel z-10">{index + 1}</div>
...
const MiniCanvas: React.FC<{frame: Frame, layers: Layer[]}> = ({frame, layers}) => {
    ...
    return <canvas ref={canvasRef} width={64} height={64} className="image-pixelated"/>
}
