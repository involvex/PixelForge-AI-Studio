...
import { Pencil, Eraser, PaintBucket, Pipette, MousePointer2, Move, Wand, RefreshCw, BoxSelect } from 'lucide-react';
...
const Toolbar: React.FC<ToolbarProps> = ({
  ...
  const tools = [
    ...
    { type: ToolType.BUCKET, icon: <PaintBucket size={20} />, label: 'Fill (B)' },
    { type: ToolType.PICKER, icon: <Pipette size={20} />, label: 'Picker (I)' },
    { type: ToolType.MAGIC_WAND, icon: <Wand size={20} />, label: 'Magic Wand (W)' },
    { type: ToolType.SELECT, icon: <MousePointer2 size={20} />, label: 'Select (S)' },
    { type: ToolType.MOVE, icon: <Move size={20} />, label: 'Move (M)' },
    { type: ToolType.TRANSFORM, icon: <BoxSelect size={20} />, label: 'Transform (T)' },
  ];
    ...
    <div className="w-16 bg-gray-850 border-r border-gray-750 flex flex-col items-center py-4 gap-4 z-10 overflow-y-auto scrollbar-none">
      ...
      <div className="mt-auto flex flex-col items-center gap-3 mb-2 pt-4 border-t border-gray-800 w-full">
        <div className="relative group">
          <label className="block text-[10px] text-gray-500 mb-1 text-center font-bold">PRI</label>
          <input
            type="color"
            value={primaryColor}
        ...
        <div className="relative group">
          <label className="block text-[10px] text-gray-500 mb-1 text-center font-bold">SEC</label>
          <input
            type="color"
